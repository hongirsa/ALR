...
var imgname= export_collection.toList(export_collection.size()).map(function(image){ return ee.Image(image).id()})
//print (imgname.get(0).getInfo()+"_"+siteSelect+"_"+outputName)
var image_output_names=imgname.get(0).getInfo()+"_"+siteSelect+"_"+outputName;
print (image_output_names)
Export.image.toAsset({image:export_collection.first(),
					            description : image_output_names,
					            assetId : assetfolder+image_output_names,
					            region : mapBounds,
					            scale :10,
					            maxPixels :1e13})
...
*/
//# parse the networks
var colName_10m = 'COPERNICUS/S2_SR_10m'
var colOptions_10m = COLLECTION_OPTIONS[colName_10m]
var netOptions_10m = VIS_OPTIONS[outputName][colName_10m]

//var numNets = ee.Number(ee.Feature((COLLECTION_OPTIONS[colName]["Network_Ind"]).first()).propertyNames().remove('Feature Index').remove('system:index').remove('lon').size());
var numNets_10m = ee.Number(ee.Feature(COLLECTION_OPTIONS[colName_10m].Network_Ind.first()).propertyNames().remove('Feature Index').remove('system:index').remove('lon').size());
var SL2P_10m = ee.List.sequence(1, ee.Number(COLLECTION_OPTIONS[colName_10m]["numVariables"]), 1).map(Nets.makeNetVars.bind(null, COLLECTION_OPTIONS[colName_10m].Collection_SL2P, numNets_10m));
//print (SL2P)
var errorsSL2P_10m = ee.List.sequence(1, ee.Number(COLLECTION_OPTIONS[colName_10m]["numVariables"]), 1).map(Nets.makeNetVars.bind(null, COLLECTION_OPTIONS[colName_10m].Collection_SL2Perrors, numNets_10m));

var input_collection_10m = ee.ImageCollection(testImage)
	.map(addDate)
	.map(function(image) { return image.clip(mapBounds) })
	.map(s2MaskClear)
	.map(s2MaskLand)
	.map(addS2Geometry.bind(null, colOptions_10m))

// get partition used to select network
var partition_10m = (COLLECTION_OPTIONS[colName_10m]["partition"]).filterBounds(mapBounds).mosaic().clip(mapBounds).rename('partition')

// pre process input imagery and flag invalid inputs
var scaled_input_collection_10m = input_collection_10m.map(Utils.scaleBands.bind(null, netOptions_10m.inputBands, netOptions_10m.inputScaling))
	.map(Nets.invalidInput.bind(null, colOptions_10m.sl2pDomain, netOptions_10m.inputBands))

//apply networks to produce mapped parameters
var estimateSL2P_10m = scaled_input_collection_10m.map(wrapperNNets.bind(null, SL2P_10m, partition_10m, netOptions_10m, colOptions_10m, 'estimate', outputName))
//print(estimateSL2P)
var uncertaintySL2P_10m = scaled_input_collection_10m.map(wrapperNNets.bind(null, errorsSL2P_10m, partition_10m, netOptions_10m, colOptions_10m, 'error', outputName))
//print(uncertaintySL2P);
//# scale and offset mapped parameter bands
estimateSL2P_10m = estimateSL2P_10m.map(function(image) { return image.addBands(image.select("estimate" + outputName).multiply(ee.Image.constant(outputScale)).add(ee.Image.constant(outputOffset)), null, true) })
uncertaintySL2P_10m = uncertaintySL2P_10m.map(function(image) { return image.addBands(image.select("error" + outputName).multiply(ee.Image.constant(outputScale)).add(ee.Image.constant(outputOffset)), null, true) })
//# produce final export collection
var export_collection_10m = input_collection_10m.combine(estimateSL2P_10m).combine(uncertaintySL2P_10m)

print(export_collection_10m)
...
Map.addLayer(export_collection_10m.mosaic().select('estimate' + outputName), {}, 'estimate_10m' + outputName);
/*------------------------------------------------------------------------------------------------------------
------------------------------------------------------Export results------------------------------------------
--------------------------------------------------------------------------------------------------------------
*/

var imgname_10m= export_collection_10m.toList(export_collection_10m.size()).map(function(image){ return ee.Image(image).id()})
//print (imgname_10m.get(0).getInfo()+"_"+siteSelect+"_"+outputName)
var image_output_names_10m=imgname_10m.get(0).getInfo()+"_"+siteSelect+"_"+outputName;
print (image_output_names_10m)
Export.image.toAsset({image:export_collection_10m.first(),
					            description : image_output_names_10m,
					            assetId : assetfolder+image_output_names_10m,
					            region : mapBounds,
					            scale :10,
					            maxPixels :1e13})
...
/*----------------------------------------------------------------------------------------------------------------------
-----------------------Active Learning Regularization (LARS Feature Selection)------------------------------------------
------------------------------------------------------------------------------------------------------------------------
*/
...
Export.image.toAsset({image:rf_classified ,
					  description : siteSelect+'_'+outputName+'_rf_10m',
					  assetId : assetfolder+siteSelect+'_'+outputName+'_rf_10m',
					  region : mapBounds,
					  scale :10,
					  maxPixels :1e13})
...
var trimmedCollection = alr.trim_data(inputImage.updateMask(inputImage.select(responseBand).gt(0)),
  ...
  10000,
	10)
print(trimmedCollection.limit(10))

Export.table.toDrive({
  collection: trimmedCollection,
  description: 'trimmedCollection',
  fileFormat: 'CSV',
})
